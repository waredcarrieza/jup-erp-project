...
<div class="container">
	<div class="row mb-2">
		<div class="col-md-12 mt-2">
			<h5>EDIT BARANG INVENTORY</h5>

			<a href="{{ url('/inventori') }}" class="btn btn-sm btn-primary">BACK</a>
		</div><!-- /.col-md-4 -->
	</div><!-- /.row -->

	<form method="post" action="{{ url('/inventori/update') }}">
	@csrf
			...
			<div class="row">
				<div class="col-md-12">
					...
					<div class="mb-1 row">
					    ...
					    <div class="col-sm-4">
					      <input type="text" class="form-control form-control-sm " id="no_inventory" name="no_inventory" value="{!! $dbrow['no_inventory'] !!}">
					    </div>
					</div>
					...
					<div class="mb-1 row">
					    ...
					    <div class="col-sm-4">
					      <input type="text" name="tanggal_inventory" id="tanggal_inventory" class="form-control form-control-sm" placeholder="dd-mm-yyyy" value="{!! convertDateTime($dbrow['tanggal_inventory'], 'd-m-Y') !!}" autocomplete="off" required="" style="width: 30%;" />
					    </div>
					</div>
					...
					<div class="mb-1 row">
					    ...
					    <div class="col-sm-4">
					      <input type="text" class="form-control form-control-sm " id="no_referensi" name="no_referensi" value="{!! $dbrow['no_referensi'] !!}">
					    </div>
					</div>
					...
					<div class="mb-1 row">
					    ...
					    <div class="col-sm-4">
					      <textarea class="form-control form-control-sm " id="note" name="note" rows="4">{!! $dbrow['note'] !!}</textarea>
					    </div>
					</div>
				</div><!-- /.col-md-12 -->
			</div><!-- /.row -->
				...
				<div class="col-md-12">
					<table class="table" id="detail_item" style="font-size: 13px;">
						...
						<tbody>
							...
							if(count($dbrow_detail) > 0):
								foreach($dbrow_detail as $row):
									$start++;
							?>

							<tr id="row_{!! $start !!}">
								<td><a role="button" class="btn btn-sm btn-outline-danger text-danger removeItem removeItem_{!! $start !!}" data-id="" data-calcid="{!! $start !!}" data-subtotal=""><i class="fas fa-times"></i> </a></td>
								<td>{!! $start !!}</td>
								<td><input type="text" class="form-control form-control-sm no_produk calc" name="no_produk[]" id="no_produk_{!! $start !!}" svn="{!! $start !!}" readonly="" value="{!! $row->no_produk !!}"></td>
								<td><input type="text" class="form-control form-control-sm name name_{!! $start !!}" name="nama_produk[]" id="nama_produk_{!! $start !!}" svn="{!! $start !!}" value="{!! $row->nama_produk !!}">
									<input type="hidden" name="product_id[]" id="product_id_{!! $start !!}" value="{!! $row->product_id !!}">
							...
							<tr id="row_{!! $start !!}">
								<td><a role="button" class="btn btn-sm btn-outline-danger text-danger removeItem removeItem_{!! $start !!}" data-id="" data-calcid="{!! $start !!}" data-subtotal=""><i class="fas fa-times"></i> </a></td>
								<td>{!! $start !!}</td>
								<td><input type="text" class="form-control form-control-sm no_produk calc" name="no_produk[]" id="no_produk_{!! $start !!}" svn="{!! $start !!}" readonly=""></td>
								<td><input type="text" class="form-control form-control-sm name name_{!! $start !!}" name="nama_produk[]" id="nama_produk_{!! $start !!}" svn="{!! $start !!}">
									<input type="hidden" name="product_id[]" id="product_id_{!! $start !!}">
									<input type="hidden" name="detail_id[]" id="detail_id_{!! $start !!}" value=""></td>
								<td><input type="text" class="form-control form-control-sm text-end qty calc" name="item_qty[]" id="item_qty_{!! $start !!}" svn="{!! $start !!}"></td>
							</tr>
						</tbody>
					</table>
				</div><!-- /.col-md-12 -->
		...
		<div class="card-body">
			<div class="row">
				<div class="col-md-12 text-end">
					...
					<input type="hidden" id="inputCode" name="inputCode" value="{!! $dbrow['code'] !!}">
		...
		$('#detail_item').find('tbody').append(
			'<tr id="row_'+num+'">'+
				...
				'<td>\
					...
					<input type="hidden" name="product_id[]" id="product_id_'+num+'">\
